...
namespace AlgorithmSharp
{
    public static class StringAlgorithm
    {
            ...
            int naive = 10, int doubling = 40)
        {
            ...
            var sa = new int[n];
            var ls = new bool[n];
            for (var i = n - 2; i >= 0; i--) ls[i] = items[i] == items[i + 1] ? ls[i + 1] : items[i] < items[i + 1];
            ...
            void Induce(IEnumerable<int> ilms)
            {
                ...
                {
                    var v = sa[i];
                    if (v >= 1 && ls[v - 1]) sa[--buffer[items[v - 1] + 1]] = v - 1;
                }
            }
        ...
        private static IEnumerable<int> CreateSuffixesByDoubling(int[] items)
        {
            ...
            for (var k = 1; k < n; k *= 2)
            {
                ...
                Array.Sort(sa, Compare);
                tmp[sa[0]] = 0;
                for (var i = 1; i < n; i++) tmp[sa[i]] = tmp[sa[i - 1]] + (Compare(sa[i - 1], sa[i]) < 0 ? 1 : 0);
